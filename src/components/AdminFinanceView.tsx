        ...
        <div className="mt-6">
          ...
          {Array.isArray(data?.ratings) && data.ratings.length ? <div className="grid gap-3 lg:grid-cols-2">{data.ratings.map((x: any) => (
            <article key={x.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between"><StaticRating value={Number(x.rating)}/><button onClick={() => void removeRating(x.id)} aria-label="Moderar calificación" className="rounded-xl bg-red-50 p-2.5 text-red-600"><Trash2 size={15}/></button></div>
...
function ReportsDashboard({ data, reload }: { data: any; reload: () => Promise<void> }) {
  ...
  return <div className="mt-6 space-y-5">
    ...
    <section className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-amber-50 text-amber-600"><Star/></span><div><h3 className="font-black">Distribución de calificaciones</h3><p className="text-xs text-slate-500">Valoraciones recibidas por usuarios y asesores</p></div></div><div className="mt-5 max-h-80 space-y-2 overflow-y-auto pr-1">{[5,4.5,4,3.5,3,2.5,2,1.5,1].map(star=>{const count=Number(data?.ratings?.distribution?.find((item:any)=>Number(item.rating)===star)?.count||0), total=Number(data?.ratings?.count||0);return <div key={star} className="grid grid-cols-[42px_1fr_28px] items-center gap-3 text-xs"><span className="font-black text-amber-600">{star} ★</span><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-amber-400" style={{width:`${ratio(count,total)}%`}}/></div><span className="text-right font-bold text-slate-500">{count}</span></div>})}</div></div>
...
function StaticRating({value}:{value:number}){return <div className="flex items-center gap-2"><span className="flex gap-0.5">{[1,2,3,4,5].map(index=>{const fill=value>=index?100:value>=index-.5?50:0;return <span key={index} className="relative block size-5 text-slate-200"><Star className="absolute inset-0 size-5" fill="currentColor"/><span className="absolute inset-0 overflow-hidden text-amber-400" style={{width:`${fill}%`}}><Star className="absolute inset-0 size-5 max-w-none" fill="currentColor"/></span></span>})}</span><strong className="text-xs text-amber-700">{value}/5</strong></div>}
function Metric({
  icon: Icon,
  label,
  value,
