...
export default function DelegatedTrackingView({ procedure, caseId, onBack }: { procedure: Procedure; caseId: string; onBack: () => void }) {
  ...
  return <div className="space-y-6 animate-fadeIn">
    ...
    {data.advisor && <section ref={chatRef} className="grid gap-5 lg:grid-cols-[320px_1fr]"><aside className="rounded-[2rem] bg-slate-950 p-6 text-white"><p className="text-xs font-black uppercase tracking-widest text-cyan-300">Canal directo</p><h2 className="mt-2 text-2xl font-black">Habla con tu asesor</h2><p className="mt-3 text-sm leading-6 text-slate-300">Pregunta por avances, observaciones o el siguiente paso de tu gestión.</p><img src={data.advisor.avatarUrl} alt="" className="mt-6 size-24 rounded-3xl border-4 border-white/10 object-cover"/><p className="mt-3 font-black">{data.advisor.publicName}</p></aside><CaseMessages caseId={caseId}/></section>}
  </div>;
}
