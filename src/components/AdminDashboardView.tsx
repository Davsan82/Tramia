...
export default function AdminDashboardView({ onExit }: { onExit: () => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(result.message || "No pudimos cargar el panel.");
      setData(result);
    } catch (e) {
    ...
    return (
      <div className="min-h-screen bg-slate-100 text-slate-950">
        <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
          <div className="mx-auto flex min-h-17 max-w-[1500px] items-center gap-4 px-4 sm:px-6 lg:px-8">
            <TramIALogo
              iconSize={31}
              textSize="text-xl"
              variant="light"
    ...
    <div className="min-h-screen bg-slate-100 text-slate-950">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex min-h-17 max-w-[1500px] items-center gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <button
            onClick={onExit}
            className="ml-auto inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-xs font-black text-slate-600 hover:bg-slate-50"
          >
      ...
      <main className="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">
        <section className="relative overflow-hidden rounded-[2rem] bg-[linear-gradient(120deg,#061735,#0d4db7_60%,#13afd1)] p-6 text-white shadow-xl sm:p-8">
          <div className="absolute inset-0 opacity-20 bg-[radial-gradient(circle_at_1px_1px,white_1px,transparent_0)] bg-size-[22px_22px]" />
          <div className="relative grid items-center gap-5 md:grid-cols-[1fr_220px]">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100">
                Supervisa usuarios, contenido, consultas y operación desde una
                vista protegida.
              </p>
            </div>
            <img
              src="/assets/mascot/tramia-bot-reading.png"
              alt="TramIA revisa el panel administrativo"
              className="mx-auto hidden h-40 object-contain drop-shadow-xl md:block"
            />
          </div>
        </section>
        {loading ? (
          <div className="mt-8 flex min-h-72 items-center justify-center rounded-3xl border border-blue-100 bg-white">
          ...
          <div className="mt-8 rounded-3xl border border-red-200 bg-red-50 p-8 text-center">
            ...
            <h2 className="mt-3 text-lg font-black">Acceso no disponible</h2>
            ...
            <>
              <section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <Metric
                  ...
                  detail={`${data.summary.users.verified} con correo verificado`}
                  tone="blue"
                />
                <Metric
                  icon={FileText}
                  label="Trámites publicados"
                  value={data.summary.procedures.active}
                  detail={`${data.summary.procedures.total} en el catálogo`}
                  tone="cyan"
                />
                <Metric
                  icon={ClipboardList}
                  label="Gestiones activas"
                  value={data.summary.cases.active}
                  detail={`${data.summary.cases.completed} completadas`}
                  tone="violet"
                />
                <Metric
                  icon={MessageSquareText}
                  label="Consultas pendientes"
                  value={data.summary.messages.pending}
                  detail={`${data.summary.messages.total} recibidas`}
                  tone="amber"
                />
              </section>
              <section className="mt-6 grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
                <AdminCard title="Usuarios recientes" icon={UsersRound}>
                    ...
                    {data.recentUsers.map((user) => (
                      ...
                      >
                        ...
                        <div className="text-left sm:text-right">
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-black ${user.emailVerifiedAt ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-800"}`}
                          >
                ...
                <div className="space-y-6">
                  <AdminCard title="Módulos" icon={Settings}>
                    <div className="grid gap-3">
                      <Module
                        icon={FileText}
                        title="Contenido y trámites"
                        text="Categorías, entidades y catálogo"
                        onClick={() => setModule("catalog")}
                      />
                      <Module
                        icon={UsersRound}
                        title="Usuarios y roles"
                        text="Permisos y estado de cuentas"
                        onClick={() => setModule("users")}
                      />
                      <Module
                        icon={MessageSquareText}
                        title="Bandeja de contacto"
                        text="Consultas y seguimiento"
                        onClick={() => setModule("contact")}
                      />
                      <Module
                        icon={ShieldCheck}
                        title="Operación de trámites"
                        text="Seguimiento, alertas y excepciones"
                        onClick={() => setModule("operations")}
                      />
                      <Module
                        icon={UsersRound}
...
function Module({
  ...
  return (
    <>
      <C
        onClick={onClick}
        className="flex w-full gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-3 text-left hover:border-blue-200 hover:bg-blue-50"
      >
