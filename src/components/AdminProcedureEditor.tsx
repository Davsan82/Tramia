    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_390px]">
      ...
      <Card>
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          ...
          {kind === "steps" && <div className="space-y-3 rounded-2xl bg-blue-50 p-4"><Field label="Tipo de acción"><select className="field-input" value={form.completionMode||'manual'} onChange={e=>setForm({...form,completionMode:e.target.value})}><option value="manual">Confirmación manual</option><option value="form">Completar formulario</option><option value="upload">Adjuntar archivo</option><option value="date">Seleccionar fecha</option><option value="external">Acción en sitio externo</option></select></Field><Field label="Campos del formulario (JSON)"><textarea className="field-input min-h-24 font-mono text-xs" value={JSON.stringify(form.actionConfig||{},null,2)} onChange={e=>{try{setForm({...form,actionConfig:JSON.parse(e.target.value)})}catch{}}}/></Field><Checks form={form} setForm={setForm} names={[["requiresUserPresence","Requiere al usuario"],["canBeDelegated","Puede asumirlo el asesor"],["isPointOfNoReturn","Punto de no retorno"],["isOptional","Paso opcional"]]}/></div>}{" "}
          {kind === "sources" && (
            <Checks
              form={form}
              setForm={setForm}
