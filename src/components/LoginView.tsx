    ...
    <div className={`relative grid w-full overflow-hidden rounded-[2rem] border border-blue-100 bg-white shadow-[0_30px_100px_rgba(4,22,61,.28)] animate-fadeIn md:grid-cols-[.84fr_1.16fr] ${isSignup ? 'max-w-5xl' : 'max-w-4xl'}`}>
      ...
      <section className={`max-h-[92vh] overflow-y-auto px-5 sm:px-9 md:px-10 lg:px-12 ${isSignup ? 'py-7 md:py-9' : 'flex flex-col justify-center py-8 md:py-10'}`}>
        ...
        <form onSubmit={submit} className={`${isSignup ? 'mt-6' : 'mt-5'} space-y-4`}>
          ...
          {isSignup && <Field label="Celular" icon={Phone}><div className="grid grid-cols-1 gap-2 sm:grid-cols-[10.5rem_minmax(0,1fr)] sm:gap-3"><select value={form.phonePrefix} onChange={(event) => setForm((value) => ({ ...value, phonePrefix: event.target.value, phoneNumber: '' }))} required aria-label="País y prefijo telefónico" className="field-input field-select min-w-0 text-left font-bold text-blue-800">{PHONE_COUNTRIES.map((country) => <option key={country.prefix} value={country.prefix}>{country.flag} {country.country} {country.prefix}</option>)}</select><input type="tel" inputMode="numeric" pattern="[0-9]*" minLength={selectedPhoneCountry?.minLength} maxLength={selectedPhoneCountry?.maxLength || 15} autoComplete="tel-national" value={form.phoneNumber} onChange={(event) => setForm((value) => ({ ...value, phoneNumber: event.target.value.replace(/\D/g, '').slice(0, selectedPhoneCountry?.maxLength || 15) }))} required placeholder="999 999 999" aria-label="Número de celular" className="field-input min-w-0 font-semibold tracking-wide tabular-nums" /></div><span className="mt-1.5 block text-[10px] font-medium text-slate-500">{selectedPhoneCountry ? `${selectedPhoneCountry.flag} ${selectedPhoneCountry.country}: ${selectedPhoneCountry.minLength === selectedPhoneCountry.maxLength ? `${selectedPhoneCountry.minLength} dígitos` : `${selectedPhoneCountry.minLength} a ${selectedPhoneCountry.maxLength} dígitos`}.` : 'Selecciona un prefijo válido.'}</span></Field>}
