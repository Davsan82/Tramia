...
app.post('/api/v1/my-procedures/:caseId/steps/:stepId/complete', async (req, res) => {
  ...
  const now = new Date();
  const result = await db.transaction(async tx => {
    await tx.update(userProcedureSteps).set({ status: 'completed', completedAt: now, completedBy: session.user.id, completionSource: 'user_action', completionData, isFinalized: true, updatedAt: now }).where(eq(userProcedureSteps.id, step.instanceId));
