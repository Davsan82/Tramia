...
const defaultContactSettings={email:process.env.SUPPORT_EMAIL||'soporte@tramia.pe',phone:'(+51) 999 000 000',location:'Lima, Perú',schedule:'Lunes a viernes, 9:00 a. m. a 6:00 p. m.'};
app.get('/api/v1/public/settings',async(_req,res)=>{const rows=await getDrizzleDatabase().select().from(appSettings).where(eq(appSettings.isPublic,true));const settings=Object.fromEntries(rows.map(item=>[item.key,item.value]));res.json({settings:{contact:{...defaultContactSettings,...(settings.contact as object||{})}}});});
