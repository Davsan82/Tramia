...
const normalizeHumanText = (value: unknown) => String(value ?? '').normalize('NFC').trim();
const hasBrokenTextEncoding = (value: string) => /\uFFFD|\u00C3(?:\u00A1|\u00A9|\u00AD|\u00B3|\u00BA|\u00B1|\u0081|\u0089|\u008D|\u0093|\u009A|\u0091)|\u00C2(?:\u00B7|\u00BF|\u00A1)|\u00E2\u20AC/u.test(value);
const parseAdvisorRating = (value: unknown) => {
  const rating = Number(value ?? 0);
  return Number.isFinite(rating) && rating >= 0 && rating <= 5 && Number.isInteger(rating * 2) ? rating : null;
};
...
app.post('/api/v1/auth/register', async (req, res) => {
  try {
    ...
    const legacyPhone = splitStoredPhone(String(req.body.phone || ''));
    const phonePrefix = String(req.body.phonePrefix || legacyPhone.phonePrefix).trim();
    const phoneNumber = String(req.body.phoneNumber || legacyPhone.phoneNumber).replace(/\D/g, '');
    ...
    if (!isValidPhone(phonePrefix, phoneNumber)) return res.status(400).json({ error: 'invalid_phone', message: phoneLengthMessage(phonePrefix) });
...
app.patch('/api/v1/profile', async (req, res) => {
  const session = await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);
  if (!session) return res.status(401).json({ error: 'authentication_required', message: 'Inicia sesión para continuar.' });
  const phonePrefix = String(req.body.phonePrefix || '').trim(), phoneNumber = String(req.body.phoneNumber || '').trim();
...
app.get('/api/v1/payments/:id/receipt.pdf',async(req,res)=>{const session=await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);if(!session)return res.status(401).json({error:'authentication_required'});const [item]=await getDrizzleDatabase().select({id:paymentOrders.id,status:paymentOrders.status,amountMinor:paymentOrders.amountMinor,currency:paymentOrders.currency,paidAt:paymentOrders.paidAt,procedureTitle:procedures.title,trackingCode:userProcedures.trackingCode,email:users.email,username:users.username,firstName:userProfiles.firstName,lastName:userProfiles.lastName,cardBrand:paymentTransactions.cardBrand,cardLastFour:paymentTransactions.cardLastFour,reference:paymentTransactions.providerTransactionId}).from(paymentOrders).innerJoin(userProcedures,eq(paymentOrders.userProcedureId,userProcedures.id)).innerJoin(procedures,eq(userProcedures.procedureId,procedures.id)).innerJoin(users,eq(userProcedures.userId,users.id)).leftJoin(userProfiles,eq(userProfiles.userId,users.id)).leftJoin(paymentTransactions,eq(paymentTransactions.paymentOrderId,paymentOrders.id)).where(and(eq(paymentOrders.id,req.params.id),eq(userProcedures.userId,session.user.id))).limit(1);if(!item)return res.status(404).json({error:'payment_not_found',message:'No encontramos este pago en tu cuenta.'});if(!['paid','authorized','partially_refunded'].includes(item.status))return res.status(409).json({error:'receipt_unavailable',message:'El comprobante estará disponible cuando el pago sea confirmado.'});const pdf=createPaymentReceiptPdf({reference:item.reference||item.id,paidAt:item.paidAt,customerName:[item.firstName,item.lastName].filter(Boolean).join(' ')||item.username,customerEmail:item.email,procedureTitle:item.procedureTitle,trackingCode:item.trackingCode,cardBrand:item.cardBrand,cardLastFour:item.cardLastFour,amountMinor:item.amountMinor,currency:item.currency});const fileName=`tramia-boleta-${String(item.reference||item.id).replace(/[^a-zA-Z0-9_-]/g,'')}.pdf`;const disposition=req.query.download==='1'?'attachment':'inline';res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`${disposition}; filename="${fileName}"`);res.setHeader('Content-Length',String(pdf.length));res.send(pdf);});
...
app.post('/api/v1/my-procedures/:caseId/steps/:stepId/complete', async (req, res) => {
  ...
  if (step.procedureStatus === 'completed' && (step.isFinalized || step.status === 'completed')) return res.json({ completed: true, locked: true, alreadyCompleted: true, percentage: 100, status: 'completed', nextStepId: null });
  if (['cancelled', 'rejected'].includes(step.procedureStatus)) return res.status(409).json({ error: 'procedure_closed', message: 'Este trámite ya no admite cambios.' });
...
const defaultContactSettings={email:process.env.SUPPORT_EMAIL||'soporte@tramia.pe',phonePrefix:'+51',phoneNumber:'999000000',phone:'+51 999000000',location:'Lima, Perú',schedule:'Lunes a viernes, 9:00 a. m. a 6:00 p. m.',responseTime:'Dentro de 2 días hábiles'};
...
app.get('/api/v1/my-procedures/:id/delegated-tracking', async (req, res) => {
  const session = await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);
  if (!session) return res.status(401).json({ error: 'authentication_required' });
  const db = getDrizzleDatabase();
  const [caseItem] = await db.select({ id:userProcedures.id, title:procedures.title, trackingCode:userProcedures.trackingCode, status:userProcedures.status, mode:userProcedures.mode, progressPercentage:userProcedures.progressPercentage })
    .from(userProcedures).innerJoin(procedures,eq(userProcedures.procedureId,procedures.id))
    .where(and(eq(userProcedures.id,req.params.id),eq(userProcedures.userId,session.user.id))).limit(1);
  if (!caseItem) return res.status(404).json({ error:'procedure_not_found', message:'No encontramos este trámite en tu cuenta.' });
  const [request] = await db.select().from(delegationRequests).where(eq(delegationRequests.userProcedureId,caseItem.id)).limit(1);
  if (!request || !['paid','assigned','active','completed'].includes(request.status)) return res.status(409).json({ error:'delegation_not_active', message:'La delegación todavía no tiene un pago confirmado.' });
  const steps = await db.select({ id:userProcedureSteps.id, procedureStepId:userProcedureSteps.procedureStepId, status:userProcedureSteps.status, notes:userProcedureSteps.notes, completedAt:userProcedureSteps.completedAt, position:procedureSteps.position, title:procedureSteps.title, description:procedureSteps.description })
    .from(userProcedureSteps).innerJoin(procedureSteps,eq(userProcedureSteps.procedureStepId,procedureSteps.id))
    .where(eq(userProcedureSteps.userProcedureId,caseItem.id)).orderBy(asc(procedureSteps.position));
  const [assigned] = await db.select({ userId:advisorProfiles.userId, publicName:advisorProfiles.publicName, averageRating:advisorProfiles.averageRating, avatarStored:userProfiles.avatarUrl, identityVerificationStatus:userProfiles.identityVerificationStatus, assignmentStatus:advisorAssignments.status })
    .from(advisorAssignments).innerJoin(advisorProfiles,eq(advisorAssignments.advisorId,advisorProfiles.userId)).innerJoin(userProfiles,eq(userProfiles.userId,advisorProfiles.userId))
    .where(and(eq(advisorAssignments.delegationRequestId,request.id),inArray(advisorAssignments.status,['reserved','active','completed']))).orderBy(desc(advisorAssignments.assignedAt)).limit(1);
  const completedCount = steps.filter(item=>item.status==='completed').length;
  const progressPercentage = steps.length ? Math.round(completedCount/steps.length*100) : caseItem.progressPercentage;
  res.json({ case:{...caseItem,progressPercentage}, delegation:request, advisor:assigned?{...assigned,avatarUrl:resolveAvatarUrl(assigned.avatarStored,assigned.userId),idVerified:assigned.identityVerificationStatus==='verified'}:null, steps });
});
...
app.get('/api/v1/advisor/cases/:id', async (req, res) => {
  const advisor = await requireAdvisor(req, res); if (!advisor) return;
  const db=getDrizzleDatabase();
  const [caseItem]=await db.select({id:userProcedures.id,ownerId:userProcedures.userId,trackingCode:userProcedures.trackingCode,status:userProcedures.status,progressPercentage:userProcedures.progressPercentage,currentStepId:userProcedures.currentStepId,title:procedures.title,category:procedureCategories.name,clientName:sql<string>`trim(concat(coalesce(${userProfiles.firstName},''),' ',coalesce(${userProfiles.lastName},'')))`,clientUsername:users.username}).from(advisorAssignments).innerJoin(delegationRequests,eq(advisorAssignments.delegationRequestId,delegationRequests.id)).innerJoin(userProcedures,eq(delegationRequests.userProcedureId,userProcedures.id)).innerJoin(procedures,eq(userProcedures.procedureId,procedures.id)).innerJoin(procedureCategories,eq(procedures.categoryId,procedureCategories.id)).innerJoin(users,eq(userProcedures.userId,users.id)).leftJoin(userProfiles,eq(users.id,userProfiles.userId)).where(and(eq(userProcedures.id,req.params.id),eq(advisorAssignments.advisorId,advisor.user.id),inArray(advisorAssignments.status,['reserved','active','completed']))).limit(1);
  if(!caseItem)return res.status(404).json({error:'case_not_found',message:'No encontramos este caso entre tus asignaciones.'});
  const [steps,preparation]=await Promise.all([db.select({id:userProcedureSteps.id,procedureStepId:userProcedureSteps.procedureStepId,status:userProcedureSteps.status,notes:userProcedureSteps.notes,completedAt:userProcedureSteps.completedAt,position:procedureSteps.position,title:procedureSteps.title,description:procedureSteps.description,isPointOfNoReturn:procedureSteps.isPointOfNoReturn}).from(userProcedureSteps).innerJoin(procedureSteps,eq(userProcedureSteps.procedureStepId,procedureSteps.id)).where(eq(userProcedureSteps.userProcedureId,caseItem.id)).orderBy(asc(procedureSteps.position)),getDelegationPrerequisites(caseItem.id,caseItem.ownerId)]);
  const prerequisites=(preparation?.steps||[]).map(step=>({id:step.id,procedureStepId:step.procedureStepId,title:step.title,description:step.description,status:step.status==='completed'&&step.isFinalized?'completed':'pending_citizen',completedAt:step.completedAt||null,completedBy:'citizen'}));
  const {ownerId:_,...publicCase}=caseItem;
  res.json({case:publicCase,steps,prerequisites});
});

app.patch('/api/v1/advisor/cases/:id/progress',async(_req,res)=>res.status(410).json({error:'sequential_progress_required',message:'El avance se registra completando un paso a la vez.'}));

app.patch('/api/v1/advisor/cases/:id/progress-legacy-disabled', async (req, res) => {
  if (req) return res.status(410).json({error:'endpoint_retired',message:'Este contrato fue retirado; completa los pasos en orden.'});
...
app.post('/api/v1/advisor/cases/:id/steps/:stepInstanceId/complete', async (req,res) => {
  const advisor=await requireAdvisor(req,res); if(!advisor)return;
  const db=getDrizzleDatabase();
  const [caseItem]=await db.select({id:userProcedures.id,status:userProcedures.status,ownerId:userProcedures.userId,ownerEmail:users.email,ownerUsername:users.username,title:procedures.title,trackingCode:userProcedures.trackingCode,assignmentId:advisorAssignments.id,delegationId:delegationRequests.id})
    .from(advisorAssignments).innerJoin(delegationRequests,eq(advisorAssignments.delegationRequestId,delegationRequests.id)).innerJoin(userProcedures,eq(delegationRequests.userProcedureId,userProcedures.id)).innerJoin(users,eq(userProcedures.userId,users.id)).innerJoin(procedures,eq(userProcedures.procedureId,procedures.id))
    .where(and(eq(userProcedures.id,req.params.id),eq(advisorAssignments.advisorId,advisor.user.id),eq(advisorAssignments.status,'active'))).limit(1);
  if(!caseItem)return res.status(404).json({error:'case_not_found',message:'No encontramos este trámite entre tus asignaciones activas.'});
  const steps=await db.select({id:userProcedureSteps.id,procedureStepId:userProcedureSteps.procedureStepId,status:userProcedureSteps.status,position:procedureSteps.position,title:procedureSteps.title})
    .from(userProcedureSteps).innerJoin(procedureSteps,eq(userProcedureSteps.procedureStepId,procedureSteps.id)).where(eq(userProcedureSteps.userProcedureId,caseItem.id)).orderBy(asc(procedureSteps.position));
  const firstPending=steps.find(item=>!['completed','skipped'].includes(item.status));
  if(!firstPending)return res.status(409).json({error:'case_already_completed',message:'Este trámite ya fue completado.'});
  if(firstPending.id!==req.params.stepInstanceId)return res.status(409).json({error:'previous_step_required',message:`Primero completa el paso ${firstPending.position}: ${firstPending.title}.`});
  const notes=String(req.body.notes||'').trim().slice(0,2000),now=new Date();
  const completedCount=steps.filter(item=>['completed','skipped'].includes(item.status)).length+1;
  const finished=completedCount>=steps.length,progress=steps.length?Math.round(completedCount/steps.length*100):100;
  const next=steps.find(item=>item.position>firstPending.position&&!['completed','skipped'].includes(item.status));
  await db.transaction(async tx=>{
    await tx.update(userProcedureSteps).set({status:'completed',startedAt:now,completedAt:now,completedBy:advisor.user.id,completionSource:'advisor',notes:notes||null,isFinalized:true,updatedAt:now}).where(eq(userProcedureSteps.id,firstPending.id));
    if(next)await tx.update(userProcedureSteps).set({status:'available',updatedAt:now}).where(eq(userProcedureSteps.id,next.id));
    await tx.update(userProcedures).set({status:finished?'completed':'in_progress',progressPercentage:progress,currentStepId:next?.procedureStepId||null,completedAt:finished?now:null,updatedAt:now}).where(eq(userProcedures.id,caseItem.id));
    await tx.update(delegationRequests).set({status:finished?'completed':'active',updatedAt:now}).where(eq(delegationRequests.id,caseItem.delegationId));
    await tx.insert(procedureStatusHistory).values({userProcedureId:caseItem.id,changedBy:advisor.user.id,previousStatus:caseItem.status,newStatus:finished?'completed':'in_progress',reason:finished?'Trámite completado por el asesor.':`Paso completado: ${firstPending.title}.`,metadata:{source:'advisor_portal',stepId:firstPending.procedureStepId,progress}});
    await tx.insert(notifications).values({userId:caseItem.ownerId,userProcedureId:caseItem.id,type:finished?'procedure_completed':'procedure_step_completed',title:finished?'Tu trámite está listo para recoger':`Tu asesor completó un paso`,body:finished?`${caseItem.title} llegó al 100%. Revisa las indicaciones finales.`:`${firstPending.title} fue completado. Tu trámite tiene ${progress}% de avance.`});
    if(finished){await tx.update(advisorAssignments).set({status:'completed',endedAt:now,endReason:'Trámite completado'}).where(eq(advisorAssignments.id,caseItem.assignmentId));await tx.update(advisorProfiles).set({activeCasesCount:sql`greatest(${advisorProfiles.activeCasesCount}-1,0)`,completedCasesCount:sql`${advisorProfiles.completedCasesCount}+1`,updatedAt:now}).where(eq(advisorProfiles.userId,advisor.user.id));}
  });
  const subject=finished?`Tu trámite ${caseItem.trackingCode} está listo para recoger`:`Nuevo avance en ${caseItem.title}: ${progress}%`;
  await Promise.race([sendAccountEmail(caseItem.ownerEmail,subject,'/'),new Promise(resolve=>setTimeout(resolve,5000))]).catch(error=>console.warn('[procedure-progress-email]',error));
  res.json({updated:true,completedStepId:firstPending.id,progressPercentage:progress,status:finished?'completed':'in_progress',nextStep:next||null});
});

app.get('/api/v1/procedure-cases/:id/messages',async(req,res)=>{const session=await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);if(!session)return res.status(401).json({error:'authentication_required'});const db=getDrizzleDatabase();const [access]=await db.select({ownerId:userProcedures.userId,advisorId:advisorAssignments.advisorId}).from(userProcedures).leftJoin(delegationRequests,eq(delegationRequests.userProcedureId,userProcedures.id)).leftJoin(advisorAssignments,and(eq(advisorAssignments.delegationRequestId,delegationRequests.id),inArray(advisorAssignments.status,['reserved','active','completed']))).where(eq(userProcedures.id,req.params.id)).limit(1);if(!access||![access.ownerId,access.advisorId].includes(session.user.id))return res.status(403).json({error:'case_access_denied'});await db.update(procedureMessages).set({readAt:new Date()}).where(and(eq(procedureMessages.userProcedureId,req.params.id),eq(procedureMessages.recipientUserId,session.user.id),isNull(procedureMessages.readAt)));const messages=await db.select({id:procedureMessages.id,senderUserId:procedureMessages.senderUserId,recipientUserId:procedureMessages.recipientUserId,body:procedureMessages.body,readAt:procedureMessages.readAt,createdAt:procedureMessages.createdAt,senderUsername:users.username}).from(procedureMessages).innerJoin(users,eq(procedureMessages.senderUserId,users.id)).where(eq(procedureMessages.userProcedureId,req.params.id)).orderBy(asc(procedureMessages.createdAt));res.json({messages});});
...
app.post('/api/v1/admin/advisors',async(req,res)=>{const admin=await requireAdministrator(req,res);if(!admin)return;const userId=String(req.body.userId||''),publicName=normalizeHumanText(req.body.publicName),bio=normalizeHumanText(req.body.bio),averageRating=parseAdvisorRating(req.body.averageRating);if(!userId||publicName.length<3||bio.length<10)return res.status(400).json({error:'invalid_advisor',message:'Selecciona una cuenta y completa el nombre y la presentación.'});if(hasBrokenTextEncoding(publicName)||hasBrokenTextEncoding(bio))return res.status(400).json({error:'invalid_text_encoding',message:'El nombre o la presentación contiene caracteres dañados. Vuelve a escribir el texto antes de guardar.'});if(averageRating===null)return res.status(400).json({error:'invalid_rating',message:'La calificación debe estar entre 0 y 5 y puede avanzar de media estrella en media estrella.'});const db=getDrizzleDatabase();const [advisorRole]=await db.select().from(roles).where(eq(roles.code,'advisor')).limit(1);try{const [saved]=await db.insert(advisorProfiles).values({userId,publicName,bio,licenseNumber:normalizeHumanText(req.body.licenseNumber)||null,verificationStatus:String(req.body.verificationStatus||'pending'),availabilityStatus:String(req.body.availabilityStatus||'offline'),maxActiveCases:Math.max(1,Number(req.body.maxActiveCases)||10),baseFeeMinor:Math.max(0,Math.round(Number(req.body.baseFee||0)*100)),averageRating:averageRating.toFixed(2)}).returning();if(advisorRole)await db.insert(userRoles).values({userId,roleId:advisorRole.id}).onConflictDoNothing();await writeAdminAudit(admin.user.id,'admin.advisor.created',{userId,publicName,averageRating},req);res.status(201).json({data:saved});}catch(error:any){if(error?.code==='23505')return res.status(409).json({error:'advisor_exists',message:'Esta cuenta ya tiene un perfil de asesor.'});throw error;}});

app.patch('/api/v1/admin/advisors/:id',async(req,res)=>{const admin=await requireAdministrator(req,res);if(!admin)return;const publicName=normalizeHumanText(req.body.publicName),bio=normalizeHumanText(req.body.bio),availability=String(req.body.availabilityStatus||'offline'),verification=String(req.body.verificationStatus||'pending'),averageRating=parseAdvisorRating(req.body.averageRating);if(publicName.length<3||bio.length<10||!['available','busy','offline'].includes(availability)||!['pending','verified','rejected'].includes(verification))return res.status(400).json({error:'invalid_advisor',message:'Revisa los datos y estados del asesor.'});if(hasBrokenTextEncoding(publicName)||hasBrokenTextEncoding(bio))return res.status(400).json({error:'invalid_text_encoding',message:'El nombre o la presentación contiene caracteres dañados. Vuelve a escribir el texto antes de guardar.'});if(averageRating===null)return res.status(400).json({error:'invalid_rating',message:'La calificación debe estar entre 0 y 5 y puede avanzar de media estrella en media estrella.'});const [saved]=await getDrizzleDatabase().update(advisorProfiles).set({publicName,bio,licenseNumber:normalizeHumanText(req.body.licenseNumber)||null,verificationStatus:verification,availabilityStatus:availability,maxActiveCases:Math.max(1,Number(req.body.maxActiveCases)||10),baseFeeMinor:Math.max(0,Math.round(Number(req.body.baseFee||0)*100)),averageRating:averageRating.toFixed(2),updatedAt:new Date()}).where(eq(advisorProfiles.userId,req.params.id)).returning();if(!saved)return res.status(404).json({error:'advisor_not_found'});await writeAdminAudit(admin.user.id,'admin.advisor.updated',{userId:saved.userId,publicName:saved.publicName,averageRating},req);res.json({data:saved});});

app.patch('/api/v1/admin/advisors/:id/password',async(req,res)=>{const admin=await requireAdministrator(req,res);if(!admin)return;const password=String(req.body.password||'');if(password.length<8||password.length>128)return res.status(400).json({error:'invalid_password',message:'La contraseña debe tener entre 8 y 128 caracteres.'});const db=getDrizzleDatabase();const [advisor]=await db.select({userId:advisorProfiles.userId,publicName:advisorProfiles.publicName,username:users.username}).from(advisorProfiles).innerJoin(users,eq(users.id,advisorProfiles.userId)).where(eq(advisorProfiles.userId,req.params.id)).limit(1);if(!advisor)return res.status(404).json({error:'advisor_not_found',message:'No encontramos este asesor.'});const passwordHash=await hashPassword(password);await db.transaction(async tx=>{await tx.update(users).set({passwordHash,updatedAt:new Date()}).where(eq(users.id,advisor.userId));await tx.delete(authSessions).where(eq(authSessions.userId,advisor.userId));await tx.delete(authTokens).where(and(eq(authTokens.userId,advisor.userId),eq(authTokens.purpose,'reset_password')));});await writeAdminAudit(admin.user.id,'admin.advisor.password_changed',{advisorId:advisor.userId,username:advisor.username,sessionsRevoked:true},req);res.json({updated:true,message:`La contraseña de ${advisor.publicName} fue actualizada y sus sesiones anteriores se cerraron.`});});
