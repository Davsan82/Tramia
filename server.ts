...
app.patch('/api/v1/profile', async (req, res) => {
  ...
  const allowedPhonePrefixes = new Set(['+51', '+54', '+591', '+55', '+56', '+57', '+593', '+1', '+52', '+595', '+598', '+58', '+34']);
  const phoneLengths: Record<string, [number, number]> = { '+51': [9, 9], '+54': [10, 10], '+591': [8, 8], '+55': [10, 11], '+56': [9, 9], '+57': [10, 10], '+593': [9, 9], '+1': [10, 10], '+52': [10, 10], '+595': [9, 9], '+598': [8, 8], '+58': [10, 10], '+34': [9, 9] };
  ...
  if (!allowedPhonePrefixes.has(phonePrefix)) return res.status(400).json({ error: 'invalid_phone_prefix', message: 'Selecciona un prefijo de país válido.' });
  if (!/^\d+$/.test(phoneNumber)) return res.status(400).json({ error: 'invalid_phone', message: 'El celular debe contener solo números.' });
  const [minimumPhoneLength, maximumPhoneLength] = phoneLengths[phonePrefix];
  if (phoneNumber.length < minimumPhoneLength || phoneNumber.length > maximumPhoneLength) return res.status(400).json({ error: 'invalid_phone_length', message: minimumPhoneLength === maximumPhoneLength ? `Ingresa un celular de ${minimumPhoneLength} dígitos para el país seleccionado.` : `Ingresa un celular de ${minimumPhoneLength} a ${maximumPhoneLength} dígitos para el país seleccionado.` });
