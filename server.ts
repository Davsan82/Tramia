...
const publicUser = (user: typeof users.$inferSelect, profile: typeof userProfiles.$inferSelect | null, roleCodes: string[] = []) => ({
  ...
  avatarUrl: profile?.avatarUrl ? `/api/v1/profile/avatar/${user.id}` : '',
  roles: roleCodes,
});
...
app.post('/api/v1/my-procedures/:caseId/steps/:stepId/complete',async(req,res)=>{const session=await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);if(!session)return res.status(401).json({error:'authentication_required'});if(req.body.confirmCompleted!==true)return res.status(400).json({error:'completion_confirmation_required',message:'Confirma que deseas marcar esta etapa como realizada.'});const db=getDrizzleDatabase();const [step]=await db.select({instanceId:userProcedureSteps.id,status:userProcedureSteps.status,isFinalized:userProcedureSteps.isFinalized,ownerId:userProcedures.userId,procedureStepId:userProcedureSteps.procedureStepId,completionMode:procedureSteps.completionMode,position:procedureSteps.position}).from(userProcedureSteps).innerJoin(userProcedures,eq(userProcedureSteps.userProcedureId,userProcedures.id)).innerJoin(procedureSteps,eq(userProcedureSteps.procedureStepId,procedureSteps.id)).where(and(eq(userProcedureSteps.userProcedureId,req.params.caseId),eq(userProcedureSteps.procedureStepId,req.params.stepId))).limit(1);if(!step||step.ownerId!==session.user.id)return res.status(404).json({error:'step_not_found'});if(step.isFinalized||step.status==='completed')return res.status(409).json({error:'step_finalized',message:'Esta etapa ya fue realizada y no puede modificarse.'});const completionData=req.body.data&&typeof req.body.data==='object'?req.body.data:{};if(step.completionMode==='form'&&!Object.keys(completionData).length)return res.status(400).json({error:'step_data_required'});const now=new Date();await db.transaction(async tx=>{await tx.update(userProcedureSteps).set({status:'completed',completedAt:now,completedBy:session.user.id,completionSource:'user_action',completionData,isFinalized:true,updatedAt:now}).where(eq(userProcedureSteps.id,step.instanceId));const [next]=await tx.select({id:userProcedureSteps.id,procedureStepId:userProcedureSteps.procedureStepId}).from(userProcedureSteps).innerJoin(procedureSteps,eq(userProcedureSteps.procedureStepId,procedureSteps.id)).where(and(eq(userProcedureSteps.userProcedureId,req.params.caseId),sql`${procedureSteps.position}>${step.position}`)).orderBy(asc(procedureSteps.position)).limit(1);if(next)await tx.update(userProcedureSteps).set({status:'available',updatedAt:now}).where(eq(userProcedureSteps.id,next.id));const [counts]=await tx.select({total:sql<number>`count(*)::int`,done:sql<number>`count(*) filter(where ${userProcedureSteps.status}='completed')::int`}).from(userProcedureSteps).where(eq(userProcedureSteps.userProcedureId,req.params.caseId));const percentage=counts.total?Math.round((counts.done/counts.total)*100):0;await tx.update(userProcedures).set({progressPercentage:percentage,currentStepId:next?.procedureStepId||step.procedureStepId,status:percentage>=100?'completed':'in_progress',completedAt:percentage>=100?now:null,updatedAt:now}).where(eq(userProcedures.id,req.params.caseId));});res.json({completed:true,locked:true});});
...
app.get('/api/v1/my-procedures/by-procedure/:procedureId/workspace',async(req,res)=>{const session=await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);if(!session)return res.status(401).json({error:'authentication_required'});const db=getDrizzleDatabase();const [record]=await db.select().from(userProcedures).where(and(eq(userProcedures.userId,session.user.id),eq(userProcedures.procedureId,req.params.procedureId),inArray(userProcedures.status,[...activeProcedureStatuses]))).orderBy(desc(userProcedures.updatedAt)).limit(1);if(!record)return res.status(404).json({error:'procedure_not_found'});const [steps,requirements]=await Promise.all([db.select({procedureStepId:userProcedureSteps.procedureStepId,status:userProcedureSteps.status}).from(userProcedureSteps).where(eq(userProcedureSteps.userProcedureId,record.id)),db.select({requirementId:userProcedureRequirements.requirementId,status:userProcedureRequirements.status}).from(userProcedureRequirements).where(eq(userProcedureRequirements.userProcedureId,record.id))]);res.json({instance:record,completedStepIds:steps.filter(step=>step.status==='completed'||step.status==='skipped').map(step=>step.procedureStepId),requirements});});

app.post('/api/v1/my-procedures',async(req,res)=>{const session=await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);if(!session)return res.status(401).json({error:'authentication_required',message:'Inicia sesión para empezar un trámite.'});const procedureId=String(req.body.procedureId||'');if(!procedureId)return res.status(400).json({error:'procedure_required'});const db=getDrizzleDatabase();const [existing]=await db.select().from(userProcedures).where(and(eq(userProcedures.userId,session.user.id),eq(userProcedures.procedureId,procedureId),inArray(userProcedures.status,[...activeProcedureStatuses]))).orderBy(desc(userProcedures.updatedAt)).limit(1);if(existing)return res.json({data:existing,existing:true});const [version]=await db.select().from(procedureVersions).where(and(eq(procedureVersions.procedureId,procedureId),inArray(procedureVersions.status,['reviewed','published']))).orderBy(desc(procedureVersions.versionNumber)).limit(1);if(!version)return res.status(409).json({error:'procedure_not_available',message:'Este trámite todavía no tiene una versión disponible.'});const [steps,requirements]=await Promise.all([db.select().from(procedureSteps).where(eq(procedureSteps.procedureVersionId,version.id)).orderBy(asc(procedureSteps.position)),db.select().from(procedureRequirements).where(eq(procedureRequirements.procedureVersionId,version.id))]);const trackingCode=`TRM-${new Date().getFullYear()}-${crypto.randomUUID().slice(0,8).toUpperCase()}`;const created=await db.transaction(async tx=>{const [record]=await tx.insert(userProcedures).values({trackingCode,userId:session.user.id,procedureId,procedureVersionId:version.id,status:'active',mode:'self_service',currentStepId:steps[0]?.id||null,startedAt:new Date(),progressPercentage:0}).returning();if(steps.length)await tx.insert(userProcedureSteps).values(steps.map((step,index)=>({userProcedureId:record.id,procedureStepId:step.id,status:(index===0?'available':'locked') as 'available'|'locked'})));if(requirements.length)await tx.insert(userProcedureRequirements).values(requirements.map(requirement=>({userProcedureId:record.id,requirementId:requirement.id,status:'pending' as const})));await tx.insert(procedureStatusHistory).values({userProcedureId:record.id,changedBy:session.user.id,newStatus:'active',reason:'Trámite iniciado por el usuario.',metadata:{source:'user'}});return record});res.status(201).json({data:created,existing:false});});

app.get('/api/v1/my-procedures/:id/delegation',async(req,res)=>{const session=await findSession(parseCookie(req.headers.cookie)[SESSION_COOKIE]);if(!session)return res.status(401).json({error:'authentication_required'});const db=getDrizzleDatabase();const [record]=await db.select().from(userProcedures).where(and(eq(userProcedures.id,req.params.id),eq(userProcedures.userId,session.user.id))).limit(1);if(!record)return res.status(404).json({error:'procedure_not_found'});const [request]=await db.select().from(delegationRequests).where(eq(delegationRequests.userProcedureId,record.id)).limit(1);const orders=request?await db.select({id:paymentOrders.id,status:paymentOrders.status,amountMinor:paymentOrders.amountMinor,currency:paymentOrders.currency,provider:paymentOrders.provider,paidAt:paymentOrders.paidAt,createdAt:paymentOrders.createdAt}).from(paymentOrders).where(eq(paymentOrders.delegationRequestId,request.id)).orderBy(desc(paymentOrders.createdAt)):[];const advisors=await db.select({userId:advisorProfiles.userId,publicName:advisorProfiles.publicName,bio:advisorProfiles.bio,averageRating:advisorProfiles.averageRating,completedCasesCount:advisorProfiles.completedCasesCount,baseFeeMinor:advisorProfiles.baseFeeMinor,currency:advisorProfiles.currency,activeCasesCount:advisorProfiles.activeCasesCount,maxActiveCases:advisorProfiles.maxActiveCases}).from(advisorProfiles).where(and(eq(advisorProfiles.verificationStatus,'verified'),eq(advisorProfiles.availabilityStatus,'available'),sql`${advisorProfiles.activeCasesCount}<${advisorProfiles.maxActiveCases}`)).orderBy(desc(advisorProfiles.averageRating));res.json({procedure:record,delegation:request||null,payments:orders,advisors});});
...
app.get('/api/v1/admin/reports/overview',async(req,res)=>{const admin=await requireAdministrator(req,res);if(!admin)return;const db=getDrizzleDatabase();const [paymentsReport,ratingsReport,advisorReport,documentsReport]=await Promise.all([db.select({count:sql<number>`count(*)::int`,amount:sql<number>`coalesce(sum(${paymentOrders.amountMinor}) filter(where ${paymentOrders.status}='paid'),0)::int`}).from(paymentOrders),db.select({count:sql<number>`count(*)::int`,average:sql<string>`coalesce(round(avg(${ratings.rating})::numeric,2),0)`}).from(ratings),db.select({total:sql<number>`count(*)::int`,available:sql<number>`count(*) filter(where ${advisorProfiles.availabilityStatus}='available')::int`}).from(advisorProfiles),db.select({total:sql<number>`count(*)::int`,pending:sql<number>`count(*) filter(where ${uploadedDocuments.status}='pending')::int`}).from(uploadedDocuments).where(isNull(uploadedDocuments.deletedAt))]);res.json({payments:paymentsReport[0],ratings:ratingsReport[0],advisors:advisorReport[0],documents:documentsReport[0]});});
